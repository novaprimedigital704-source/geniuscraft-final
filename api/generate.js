...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isDisposableEmail(userEmail)) {
      return res.status(403).json({ error: "Emails descartáveis não são permitidos nesta plataforma." });
    }
    ...
    const payload = {
      ...
      parameters: {
        ...
        negative_prompt: options?.negative_prompt || "low quality, blurry, deformed, watermark, text, bad anatomy, extra limbs",
      },
    };
    ...
    return res.status(200).json({ imageUrl: `data:image/png;base64,${base64}`, credits: credits - 1 });
